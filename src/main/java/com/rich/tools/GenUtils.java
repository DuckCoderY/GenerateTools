...
public class GenUtils {
    ...
    public static List<String> getTemplates() {
        ...
        templates.add("templates/bean/ServiceImpl.java.vm");

        return templates;
    }


    public static void generatorCode(CatTable table, List<CatColumn> columns, ZipOutputStream zip, Config configH5, AtomicBoolean beenBoolean,AtomicBoolean convertBoolean) {
        //配置信息
...
//        String className = tableToJava(tableEntity.getTableName(), config.getStringArray("tablePrefix"));
        ...
        for (String template : templates) {
            ...
            if ("templates/bean/ResponseBean.java.vm".equals(template) && beenBoolean.get()) {
                continue;
            }
            if ("templates/bean/ResponseBean.java.vm".equals(template) && !beenBoolean.get()) {
                beenBoolean.set(true);
            }

            if ("templates/bean/DataConvertUtils.java.vm".equals(template) && convertBoolean.get()) {
                continue;
            }
            if ("templates/bean/DataConvertUtils.java.vm".equals(template) && !convertBoolean.get()) {
                convertBoolean.set(true);
            }
